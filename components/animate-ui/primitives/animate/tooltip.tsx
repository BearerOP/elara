...
function TooltipOverlay() {
  ...
  const { ref: _, ...contentPropsWithoutRef } = rendered.data?.contentProps || {};
...
function TooltipTrigger({
  ...
  // Helper function to merge refs (similar to Slot's mergeRefs)
  const mergeRefs = React.useCallback(
    (...refs: (React.Ref<HTMLDivElement> | undefined)[]): React.RefCallback<HTMLDivElement> => {
      return (node) => {
        refs.forEach((ref) => {
          if (!ref) return;
  ...
  // When using Slot (asChild), pass ref through children, not as a prop
  if (asChild && props.children) {
    const { ref: propsRef, ...restProps } = props as { ref?: React.Ref<HTMLDivElement> } & typeof props;
    return (
      <Slot
      ...
      >
        {React.cloneElement(props.children as React.ReactElement, {
          ref: mergeRefs(triggerRef, propsRef),
        })}
      </Slot>
    );
  }
